...
export default function StudioBatchPage() {
  ...
  const { data: claimSignals = [] } = useQuery({
    queryKey: ['studio', 'batch', batchId, 'claim-signals'],
  ...
  const invalidate = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ['studio', 'batch', batchId, 'claim-signals'] })
  }
  ...
  const applyClaimSignal = useMutation({
    mutationFn: (passportId) => studioService.applyClaimSignal(batchId, passportId),
    onSuccess: invalidate,
  })
  ...
  const generate = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['studio', 'passports'] })
    },
  ...
  const pendingSignals = claimSignals.filter((s) => !s.handled)
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <h2 className="h5 mb-1">{batch.name}</h2>
      ...
      {pendingSignals.length > 0 && (
        <FangPanel className="mb-3 border-warning">
          <h3 className="h6 mb-2">{t('studio.claimSignalsTitle')}</h3>
          <p className="small text-muted mb-2">{t('studio.claimSignalsHint')}</p>
          <ul className="list-unstyled mb-0 small">
            {pendingSignals.map((signal) => (
              <li key={signal.eventId} className="d-flex flex-wrap align-items-center justify-content-between gap-2 py-2 border-bottom">
                <div>
                  <span className="font-monospace">{signal.shortId}</span>
                  <span className="text-muted ms-2">
                    {signal.claimedAt ? new Date(signal.claimedAt).toLocaleString() : ''}
                  </span>
                </div>
                <button
                  type="button"
                  className="btn btn-sm btn-outline-warning"
                  disabled={applyClaimSignal.isPending}
                  onClick={() => applyClaimSignal.mutate(signal.passportId)}
                >
                  {t('studio.claimSignalApply')}
                </button>
              </li>
            ))}
          </ul>
      ...
      <FangPanel className="mb-3">
        <h3 className="h6 mb-2">{t('studio.adjustInventory')}</h3>
      ...
      <h3 className="h6 mb-2">{t('studio.passportsListTitle')}</h3>
      {passports.length === 0 ? (
        ...
        <FangPanel>
          ...
          <Link to={`/studio/labels?batch=${batchId}`} className="btn btn-sm btn-outline-dark mt-3">
