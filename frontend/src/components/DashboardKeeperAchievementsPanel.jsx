...
function MiniRing({ score }) {
  ...
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" className="ta-dash-summary__ring-svg" aria-hidden="true">
...
export default function DashboardKeeperAchievementsPanel({
  ...
  const visibleBadges = badgeRows.slice(0, MAX_VISIBLE_BADGES)
  const overflowCount = Math.max(0, badgeRows.length - MAX_VISIBLE_BADGES)

  return (
    <>
      {/* ── Compact inline summary ──────────────────────────────── */}
      <div className="ta-dash-summary">
        {reputation && (
          <div className="ta-dash-summary__rank">
            <MiniRing score={score} />
            <span className="ta-dash-summary__tier">{tierLabel}</span>
            <span className="ta-dash-summary__score">{score}%</span>
          </div>
        )}

        {visibleBadges.length > 0 && (
          <div className="ta-dash-summary__badge-reel">
            ...
            {overflowCount > 0 && (
              <span className="ta-dash-summary__overflow">+{overflowCount}</span>
            )}
          </div>
        )}
        ...
        <button
          type="button"
          className="ta-dash-summary__cta"
          onClick={() => setDrawerOpen(true)}
          aria-expanded={drawerOpen}
        >
          {t('dashboard.viewAchievements', { defaultValue: 'Ver logros' })}
        </button>
      </div>
      ...
      {drawerOpen && (
        <div
          className="ta-dash-drawer-overlay"
          onClick={closeDrawer}
          role="presentation"
        >
          <div
            className="ta-dash-drawer"
            role="dialog"
            aria-modal="true"
            aria-label={t('dashboard.achievementsTitle', { defaultValue: 'Logros' })}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="ta-dash-drawer__header">
              <span className="ta-dash-drawer__title">
                {t('dashboard.achievementsTitle', { defaultValue: 'Logros' })}
                {badgeRows.length > 0 && (
                  <span className="ta-dash-drawer__count">{badgeRows.length}</span>
                )}
              </span>
              <button
                type="button"
                className="ta-dash-drawer__close"
                onClick={closeDrawer}
